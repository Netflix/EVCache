...
public class EVCacheClientPoolImpl implements Runnable, EVCacheClientPoolImplMBean, EVCacheClientPool {
    ...
    private final Map<String, DynamicBooleanProperty> writeOnlyFastPropertyMap = new ConcurrentHashMap<String, DynamicBooleanProperty>() {
        public DynamicBooleanProperty get(Object zone) {
            ...
            isZoneInWriteOnlyMode = DynamicPropertyFactory.getInstance().getBooleanProperty(_appName + "." + zone.toString() + ".EVCacheClientPool.writeOnly", false);
	...
	public EVCacheClient getEVCacheClient() {
        if(memcachedReadInstancesByZone.isEmpty()) return null;
	...
	public EVCacheClient[] getAllEVCacheClients() {
        try {
            if(_zoneAffinity.get()) {
                ...
                if(clients == null)   return new EVCacheClient[0]; //GLOBAL only one zone so hardcoding it to 0
            ...
            if(activeServerCount != sizeInDiscovery || inActiveServerCount > 0) {
                ...
                for(String instance : discoveredHostsInZone) {
                    ...
                    if(!client.getConnectionObserver().getActiveServerInfo().containsKey(hostname) && !client.getConnectionObserver().getInActiveServerInfo().containsKey(hostname)) {
                        if(log.isDebugEnabled()) log.debug("AppName :" + _appName + "; Zone : " + zone + "; instance : " + instance + " not found and will shutdown the client and init it again.");
                        return true;
                    }
                }
            }
    ...
    private void setupNewClientsByZone(String zone, List<EVCacheClientImpl> newClients) {
        ...
        if(log.isDebugEnabled()) log.debug("Replaced an existing Pool for zone : " + zone + "; and app " + _appName + " ;\n\tOldClients : " + currentClients + ";\n\tNewClients : " + newClients); 
        for(EVCacheClient client : currentClients) {
            if(!client.isShutdown()) {
                if(log.isDebugEnabled()) log.debug("Shutting down in Fallback -> AppName : " + _appName + "; Zone : " + zone + "; client {" + client + "};" );
                try {
        ...
        try {
            ...
            for(Entry<String, List<String>> zoneEntry : instances.entrySet()) {
                ...
                if(instanceChangeInZone) {
                    if(log.isWarnEnabled()) log.warn("FORCE REFRESH :: AppName :" + _appName + "; Zone : " + zone + "; Changed : " + instanceChangeInZone);
                } else {
                    ...
                    if(!instanceChangeInZone) {
                        ...
                        if(log.isDebugEnabled()) log.debug("AppName :" + _appName + "; Zone : " + zone + "; Changed : " + instanceChangeInZone);
                        continue;
                    }
                }
                ...
                for(int i = 0; i < poolSize; i++) {
                    ...
                    if(log.isDebugEnabled()) log.debug("AppName :" + _appName + "; Zone : " + zone + "; intit : client.getId() : " + client.getId());
                }
    ...
    private Map<String, List<String>> discoverInstances() throws IOException {
        ...
        if (app == null )  return Collections.<String, List<String>>emptyMap();
        ...
        for (InstanceInfo iInfo : appInstances) {
            ...
            if (DataCenterInfo.Name.Amazon != dcInfo.getName()) {
                if(log.isErrorEnabled()) log.error("This is not a AmazonDataCenter. Cannot proceed. DataCenterInfo : " + dcInfo);
                continue;
            }

            /* Don't try to use downed instances */
            if (InstanceStatus.UP != iInfo.getStatus()) {
                if(log.isWarnEnabled()) log.warn("The Status of the instance in Discovery is not UP. InstanceInfo : " + iInfo);
                continue;
            }
            ...
            if(hostName == null) {
                if(log.isErrorEnabled()) log.error("The public hostnanme is null, will not be able to add this host to the evcache cluster. AmazonInfo : " + amznInfo);
                continue;
            }

            if(!instancesSpecific.containsKey(zone)) instancesSpecific.put(zone, new ArrayList<String>());
    ...
    public void shutdown() {
        if(log.isInfoEnabled()) log.info("EVCacheClientPool for App : " + _appName + " and Zone : " + _zone + " is being shutdown.");
        _shutdown = true;
        _scheduler.shutdown();
