...
public class EVCacheTestDI extends DIBase implements EVCacheGetOperationListener<String> {
    ...
    private String appName = "EVCACHE_PERF_SP_166";
...
//            test.testAll();
    ...
    public EVCacheTestDI() {
        ...
        propertiesToSet.putIfAbsent("EVCACHE_PERF_SP_166.hash.key", "true");
        propertiesToSet.putIfAbsent("evcache_perf_sp_166-useast1e-1595488843-00.hash.key", "true");
        propertiesToSet.putIfAbsent("evcache_perf_sp_166-useast1e-1595488843-00.hash.algo", "murmur3");
        propertiesToSet.putIfAbsent("evcache_perf_sp_166-useast1c-1595488843-00.hash.key", "false");
    ...
    public void testEVCache() {
        this.evCache = getInternalNewBuilder().setAppName(appName).setCachePrefix("cid").enableRetry().setDefaultTTL(86400).build();
        assertNotNull(evCache);

        try {
//            for (int i = 1; i <= 10; i++) {
//                evCache.set("sumanthkey" + i, "sumanthvalue" + i);
//            }
            Map<MemcachedNode, EVCacheInternal.CachedValues> map = ((EVCacheInternal) this.evCache).metaGetPerClient("LDKRwcmKDWc", new ByteArrayTranscoder(), true);
            map = ((EVCacheInternal) this.evCache).metaGetPerClient("sumanthkey1", new ByteArrayTranscoder(), false);
            Map<MemcachedNode, EVCacheItemMetaData> map2 = ((EVCacheInternal) this.evCache).metaDebugPerClient("sumanthkey1", false);
            int x = 10;
            x++;
        } catch (Exception ex){
